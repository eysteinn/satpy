...
class OsiSafSSTFileHandler(BaseFileHandler):

    def __init__(self, filename, filename_info, filetype_info):
        super(OsiSafSSTFileHandler, self).__init__(filename, filename_info,
                                                      filetype_info)
#        print('>>>', self.filename, '<<<<')
...
#           print(v)
        self.filename_info['start_time'] = datetime.strptime(ds.getncattr('start_time'), "%Y%m%dT%H%M%SZ")
        self.filename_info['end_time'] = datetime.strptime(ds.getncattr('stop_time'), "%Y%m%dT%H%M%SZ")
...
#        print("Getting Dataset:", stdname, key)
        if stdname in ['latitude', 'longitude']:
#            print("Getting Lat/Long", key)
...
#            print(self.ds['sea_surface_temperature'][0,:,:].shape)
            return Dataset(self.ds['sea_surface_temperature'][0,:,:], id=key, **info)
